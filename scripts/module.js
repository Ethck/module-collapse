...
Hooks.on("renderModuleManagement", async (app, html, options) => {
    ...
    Object.keys(groups).forEach((group) => {
        ...
        if (GMETHOD === "author") {
            ...
            title = toTitleCase(
                findLongestPrefix(items.map((item) => $(item).find(".package-title").text().trim().toLowerCase()))
            );
        }
        ...
        html.find("#module-list #" + titleID).prepend(
            "<div class='flexrow'><input type='checkbox' class='module-collapse-checkbox' name='" +
                titleID +
                "' data-dtype='Boolean'><label class='module-collapse-title package-title'><span id='module-count'></span>" +
                title +
                "</label><button type='button' class='module-collapse-collapsible open'>Show</button><button type='button' class='module-collapse-collapsible close'>Hide</button></div>"
        );
        ...
        moduleSubList
            .siblings("div")
            .find("label span#module-count")
            .text("(" + moduleSubList.find("li").length + ") ");
    });
